...
message(STATUS "Mengshee KF6 module build")
...
if(NOT DEFINED SKIP_CHECKOUT OR NOT SKIP_CHECKOUT)
    mengshee_ensure_git_checkout("${GIT_URL}" "${GIT_REF}" "${MODULE_SOURCE}")
endif()
if(DEFINED CLEAN AND CLEAN)
    mengshee_remove_inside("${MODULE_BUILD}" "${BUILD_ROOT}")
endif()
mengshee_prepare_build_dir("${MODULE_BUILD}" "${BUILD_ROOT}" "${MODULE_SOURCE}")
...
if(NOT DEFINED SKIP_BUILD OR NOT SKIP_BUILD)
    ...
    mengshee_run_vs("\"${CMAKE_PROGRAM}\" --build \"${MODULE_BUILD}\" --target install --parallel ${JOBS}")
    set(_module_was_built ON)
endif()

if(_module_was_built)
    set(_manifest "${SDK_PREFIX}/mengshee-sdk-modules.cmake")
    file(APPEND "${_manifest}"
"set(MENGSHEE_KF6_MODULE_${MODULE}_GIT_URL \"${GIT_URL}\")
set(MENGSHEE_KF6_MODULE_${MODULE}_GIT_REF \"${GIT_REF}\")
set(MENGSHEE_KF6_MODULE_${MODULE}_BUILD_TYPE \"${BUILD_TYPE}\")
")
